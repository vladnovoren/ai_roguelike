...
static void create_monster_explorer_beh(
    flecs::entity ent,
    Position base_position,
    const std::function<
        BehNode *(const std::vector<std::pair<BehNode *, utility_function>> &)>
        &utility_selector_gtor) {
  Blackboard bb;
  size_t base_position_id = bb.regName<Position>("base_position");
  bb.set(base_position_id, base_position);
  ent.set(Blackboard{bb});

  auto root = utility_selector_gtor(
      {std::make_pair(random_move(), [](Blackboard &) { return 50.0f; }),
       std::make_pair(move_to_entity(ent, "base"),
                      [](Blackboard &bb) {
                        auto ally_dist = bb.get<float>("allyDist");
                        auto base_dist = bb.get<float>("baseDist");
...
static void register_roguelike_systems(flecs::world &ecs) {
  ...
  ecs.system<const Position, const Hitpoints>().each([&](const Position &pos,
    ...
    const Rectangle underRect = {float(pos.x + hpPadding), float(pos.y - 0.25f),
                                 hpWidth, 0.1f};
    DrawRectangleRec(underRect, BLACK);
    const Rectangle hpRect = {float(pos.x + hpPadding), float(pos.y - 0.25f),
                              hp.hitpoints / 100.f * hpWidth, 0.1f};
...
void init_roguelike(flecs::world &ecs) {
  ...
  create_fuzzy_monster_beh(create_monster(
      ecs, 10, -5, Color{0xee, 0x00, 0xee, 0xff}, "minotaur_tex"));
  create_fuzzy_monster_beh(create_monster(
...
static void gather_world_info(flecs::world &ecs) {
  ...
  gatherWorldInfo.each([&](Blackboard &bb, const Position &pos,
    ...
    auto base_pos = bb.get<Position>("base_position");
    ...
    push_info_to_bb(bb, "baseDist", dist(base_pos, pos));
  });
}
