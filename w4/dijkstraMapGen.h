...
namespace dmaps
{
  void gen_player_approach_map(flecs::world &ecs, std::vector<float> &map, int range = 0);
  ...
  void gen_ally_map(flecs::world &ecs, std::vector<float> &map, flecs::entity target);
};
