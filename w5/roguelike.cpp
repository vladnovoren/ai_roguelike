...
static void register_roguelike_systems(flecs::world &ecs) {
  ...
  ecs.system<const Position, const Color>()
      ...
      .each([&](flecs::entity e, const Position &pos, const Color color) {
        ...
        DrawTextureQuad(
            *textureSrc.get<Texture2D>(), Vector2{1, 1}, Vector2{0, 0},
            Rectangle{float(pos.x) * tile_size, float(pos.y) * tile_size,
                      tile_size, tile_size},
            color);
      });
  ecs.system<const Position, const Color>()
      ...
      .each([&](const Position &pos, const Color color) {
        const Rectangle rect = {float(pos.x) * tile_size,
                                float(pos.y) * tile_size, tile_size, tile_size};
  ...
  ecs.system<const Position, const Hitpoints>().each([&](const Position &pos,
                                                         const Hitpoints &hp) {
    constexpr float hpPadding = 0.05f;
    const float hpWidth = 1.f - 2.f * hpPadding;
    const Rectangle underRect = {float(pos.x + hpPadding) * tile_size,
                                 float(pos.y - 0.25f) * tile_size,
                                 hpWidth * tile_size, 0.1f * tile_size};
    DrawRectangleRec(underRect, BLACK);
    const Rectangle hpRect = {
        float(pos.x + hpPadding) * tile_size, float(pos.y - 0.25f) * tile_size,
        hp.hitpoints / 100.f * hpWidth * tile_size, 0.1f * tile_size};
    DrawRectangleRec(hpRect, RED);
  });

  ecs.system<Texture2D>().each(
      [&](Texture2D &tex) { SetTextureFilter(tex, TEXTURE_FILTER_POINT); });
  ecs.system<const DmapWeights>().term<VisualiseMap>().each(
      [&](const DmapWeights &wt) {
          ...
          for (size_t y = 0; y < dd.height; ++y)
            for (size_t x = 0; x < dd.width; ++x) {
              ...
              for (const auto &pair : wt.weights) {
                ecs.entity(pair.first.c_str())
                    .get([&](const DijkstraMapData &dmap) {
              ...
              if (sum < 1e5f)
                DrawText(TextFormat("%.1f", sum),
                         int((float(x) + 0.2f) * tile_size),
                         int((float(y) + 0.5f) * tile_size), 150, WHITE);
            }
        });
      });
  ecs.system<const DijkstraMapData>().term<VisualiseMap>().each(
      [](const DijkstraMapData &dmap) {
        dungeonDataQuery.each([&](const DungeonData &dd) {
          for (size_t y = 0; y < dd.height; ++y)
            for (size_t x = 0; x < dd.width; ++x) {
...
void init_roguelike(flecs::world &ecs) {
  ...
  create_hive_monster(
      create_monster(ecs, Color{0xee, 0x00, 0xee, 0xff}, "minotaur_tex"));
  create_hive_monster(
      create_monster(ecs, Color{0xee, 0x00, 0xee, 0xff}, "minotaur_tex"));
  create_hive_monster(
      create_monster(ecs, Color{0x11, 0x11, 0x11, 0xff}, "minotaur_tex"));
  create_hive(create_player_fleer(
      create_monster(ecs, Color{0, 255, 0, 255}, "minotaur_tex")));
...
void init_dungeon(flecs::world &ecs, char *tiles, size_t w, size_t h) {
  flecs::entity wallTex =
      ecs.entity("wall_tex").set(Texture2D{LoadTexture("assets/wall.png")});
  flecs::entity floorTex =
      ecs.entity("floor_tex").set(Texture2D{LoadTexture("assets/floor.png")});

  std::vector<char> dungeonData;
  dungeonData.resize(w * h);
  for (size_t y = 0; y < h; ++y)
    for (size_t x = 0; x < w; ++x) dungeonData[y * w + x] = tiles[y * w + x];
...
static void push_to_log(flecs::world &ecs, const char *msg) {
  ...
  queryLog.each([&](ActionLog &l, const TurnCounter &c) {
    ...
    if (l.log.size() > l.capacity) l.log.erase(l.log.begin());
  });
}
...
static void process_actions(flecs::world &ecs) {
  ...
  ecs.defer([&] {
    processHeals.each([&](Action &a, Hitpoints &hp) {
      if (a.action != EA_HEAL_SELF) return;
      a.action = EA_NOP;
...
static void gather_world_info(flecs::world &ecs) {
  static auto gatherWorldInfo =
      ecs.query<Blackboard, const Position, const Hitpoints,
                const WorldInfoGatherer, const Team>();
  static auto alliesQuery = ecs.query<const Position, const Team>();
  gatherWorldInfo.each([&](Blackboard &bb, const Position &pos,
                           const Hitpoints &hp, WorldInfoGatherer,
                           const Team &team) {
...
void process_turn(flecs::world &ecs) {
  ...
  if (is_player_acted(ecs)) {
    ...
    ecs.entity("hive_map").set(DijkstraMapData{hiveMap});

    // ecs.entity("flee_map").add<VisualiseMap>();
    ecs.entity("hive_follower_sum")
        .set(DmapWeights{
            {{"hive_map", {1.f, 1.f}}, {"approach_map", {1.8f, 0.8f}}}})
        .add<VisualiseMap>();
  }
}
